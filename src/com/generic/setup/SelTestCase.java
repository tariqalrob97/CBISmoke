...
public class SelTestCase {
	...
	/**
	 * tearDown function will be invoked by Junit after execution of every test
	 * case. It adds the test case status into the html report and close the
	 * browser. It reads email id from config file. If they are mentioned then
	 * framework will zip the html reports and send it into corresponding email ids.
	 * 
	 * @throws Exception
	 */
	@AfterMethod
	...
	public static void reportMaker(XmlTest test) throws IOException {
		...
		for (HashMap<String, String> status : casesDetails) {
			...
			if (!(status.get("TestName").equals("") || status.get("StartTime").equals("")
					|| status.get("EndTime").equals("")))
				ReportUtil.addTestCase(status.get("TestName"), status.get("Details"), status.get("StartTime"),
						status.get("EndTime"), status.get("Status"), status.get("LogFileName"), status.get("Browser"));
		} // for
